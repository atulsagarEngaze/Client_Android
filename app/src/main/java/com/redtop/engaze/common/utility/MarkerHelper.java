...
public class MarkerHelper {
	...
	public static Marker drawParticipantMarker(LatLng latLng, UsersLocationDetail userLocationDetail, GoogleMap map, Context context){
		...
		try {
			...
			bitmap1 = BitMapHelper.getCroppedBitmap( Bitmap.createScaledBitmap(userLocationDetail.getContactOrGroup().getImageBitmap(context), circleSize, circleSize, true)) ;
	...
	public static Marker drawTimeDistanceMarker(LatLng latLng, UsersLocationDetail ud, GoogleMap map, Context context){
		latLng = new LatLng(latLng.latitude, latLng.longitude);
		View etaMarker = ((BaseActivity1)context).getLayoutInflater().inflate(R.layout.custom_snippet_eta, null);
	...
	public static Marker drawPinMarker(GoogleMap map, LatLng latLng, Context context){
		try {
			...
			Bitmap centerIcon = BitMapHelper.generateCircleBitmapForText(context, context.getResources().getColor(R.color.primary), 40, "");
